...
=======
- Students are now modelled as a separate aggregate with `/api/students` CRUD endpoints, CQRS handlers, migrations, and seed data per flagship university.
...
=======
- Dedicated `/courses` and `/students` pages provide CRUD experiences for each entity, sharing the component library tables + modal forms (including delete confirmations).
>>>>>>> edf7032 (feat: add students feature)
