...
public sealed record CreateInstitutionCommand(string Name, string Country, string StateProvince, int Enrollment) : IRequest<Guid>;
...
public sealed class CreateInstitutionCommandHandler : IRequestHandler<CreateInstitutionCommand, Guid>
{
    ...
    {
        var institution = new Institution(request.Name, request.Country, request.StateProvince, request.Enrollment);
        await _repository.AddAsync(institution, cancellationToken);
        await _unitOfWork.SaveChangesAsync(cancellationToken);

        var @event = new InstitutionChangedEvent(institution.Id, institution.Name, institution.Country, institution.StateProvince, institution.Enrollment, "Created");
